...
class image_dehazer():
    ...
    def __circularConvFilt(self, Img, Filter):
        ...
        filterHalfSize = int((FilterHeight - 1) / 2)
        rows, cols = Img.shape
        PaddedImg = cv2.copyMakeBorder(Img, filterHalfSize, filterHalfSize, filterHalfSize, filterHalfSize,
                                       borderType=cv2.BORDER_WRAP)
        FilteredImg = cv2.filter2D(PaddedImg, -1, Filter)
        Result = FilteredImg[filterHalfSize:rows + filterHalfSize, filterHalfSize:cols + filterHalfSize]
        return (Result)
